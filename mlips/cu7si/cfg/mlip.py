...
def convert_to_db(filename, format='vasp', db_path="cfgs.db"):
    ...
    for _struct in range(len(input_files)):
        ...
        print(struct)
        try:
            filepath = os.path.join('/work/angel/WP1/cusinicr/cusi/stables/cu7si/dir-OUTCAR' + str(struct), 'OUTCAR')
            print(filepath)
            _atom = ase_read(filepath, format=format)
            atoms.append(_atom)
            print(_atom)
        except:
            filepath = os.path.join('/work/angel/WP1/cusinicr/cusi/stables/cu7si/POSCAR-' + str(struct) , 'POSCAR')
            print(filepath)
